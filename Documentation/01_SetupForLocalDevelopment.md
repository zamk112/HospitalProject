...
## Adding CORS
I'm going to add CORS to only allow certain URLs access the APIs for local development. 
```csharp
...
try
{
...
At the moment, I'm just allowing everything on the local dev setup. I don't want any funny business outside from my VM, since I'm running everything from the inside. I will need to come back later and add another url for when hosting the application stack on docker. 
